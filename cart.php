...
if (isset($_POST['checkout']) && isset($_POST['order_id'])) {
    ...
    try {
        ...
        $query = "SELECT id FROM orders WHERE id = :id AND user_id = :user_id AND status = 'pending' LIMIT 1";
        $stmt = $db->prepare($query);
        $stmt->bindParam(':id', $order_id);
        $stmt->bindParam(':user_id', $_SESSION['user_id']);
        $stmt->execute();
        ...
        // Get all order items with current book stock
        $query = "SELECT oi.book_id, oi.quantity, b.stock, b.title 
                  FROM order_items oi 
        ...
        foreach ($items as $item) {
            if ($item['stock'] < $item['quantity']) {
                $out_of_stock_items[] = $item['title'] . ' (Stok tersedia: ' . $item['stock'] . ', Dibutuhkan: ' . $item['quantity'] . ')';
            }
        }
        
        if (!empty($out_of_stock_items)) {
            throw new Exception('Stok tidak mencukupi untuk: ' . implode(', ', $out_of_stock_items));
        }
        ...
        // Update book stock
        foreach ($items as $item) {
            $update_query = "UPDATE books SET stock = stock - :quantity WHERE id = :book_id AND stock >= :quantity";
            ...
            if ($update_stmt->rowCount() == 0) {
                throw new Exception('Gagal update stok untuk buku: ' . $item['title']);
            }
        }
...
<body>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form method="POST" class="checkout-form">
                                            ...
                                            <button type="submit" name="checkout" class="btn btn-success btn-sm">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-4">
                                            ...
                                            <?php else: ?>
                                                <div style="width: 100%; aspect-ratio: 3/4; background: #f0f0f0; display: flex; align-items: center; justify-content: center; border-radius: 4px;">
                                        ...
                                        <div class="col-8">
                                            ...
                                            <p class="text-muted small mb-1"><?php echo htmlspecialchars($item['author']); ?></p>
                                            <p class="mb-1"><strong>Rp <?php echo number_format($item['price'], 0, ',', '.'); ?></strong></p>
                                            <p class="mb-2">Jumlah: <?php echo $item['quantity']; ?></p>
                                            <p class="mb-2"><strong>Subtotal: Rp <?php echo number_format($item['price'] * $item['quantity'], 0, ',', '.'); ?></strong></p>
                                            <form method="POST" class="remove-item-form d-inline">
                                                <input type="hidden" name="item_id" value="<?php echo $item['id']; ?>">
                                                <button type="submit" name="remove_item" class="btn btn-sm btn-danger">
                                                    <i class="bi bi-trash"></i> Hapus
                                                </button>
                        ...
                        <div class="card bg-light">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <h5 class="mb-0"><strong>Rp <?php echo number_format($pending_order['total_amount'], 0, ',', '.'); ?></strong></h5>
                                </div>
                                <form method="POST" class="checkout-form">
                                    <input type="hidden" name="order_id" value="<?php echo $pending_order['id']; ?>">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Handle remove item with SweetAlert - Direct button click handlers
                    ...
                    if (form) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                // Create hidden input to submit form
            ...
            // Handle checkout with SweetAlert - Direct button click handlers
            ...
            // Also handle form submit events as backup
                ...
                form.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            const formAction = formElement.action || window.location.href;
                            const formMethod = formElement.method || 'POST';
